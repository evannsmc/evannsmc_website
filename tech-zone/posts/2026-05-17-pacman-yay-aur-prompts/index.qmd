...
1. **Your stow packages should *add* files to `~/.config/`, never *replace* what the bundle installed.**
   Use new filenames (`99-evanns.fish`, `personal-binds.conf`), or use subdirectories the bundle leaves alone (`conf.d/`, `local/`, `plugins/`). If you stow a file that has the same path as a bundle symlink, stow refuses with a "would override" error — which is the *right* behavior; it's telling you to pick a non-colliding name.
