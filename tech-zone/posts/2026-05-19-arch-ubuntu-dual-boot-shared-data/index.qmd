...
A continuation of [Article 03](../2026-05-17-arch-luks-btrfs-caelestia/), turning that single-boot Arch install into a dual-boot with Ubuntu 24 LTS sharing the same NVMe drive. Concretely:
...
My [first attempt at a dual-boot with LUKS, Btrfs, zram, and a shared partition](../2026-05-05-arch-luks-btrfs-lessons/) (Article 01) tried to land everything in one swing and didn't boot. The post-mortem isolated two `cryptdevice=` typos in the kernel command line and one missing `rootflags=subvol=@` parameter. After I fixed those in [Article 03](../2026-05-17-arch-luks-btrfs-caelestia/), the single-boot version worked end-to-end.
...
3. **Resume-image collisions.** Two OSes hibernating to the same swap target would overwrite each other's resume image. We sidestep this entirely by giving each OS its own NoCOW Btrfs swap file inside its own LUKS root, plus zram for everyday compressed swap — exactly the [Article 03 pattern](../2026-05-17-arch-luks-btrfs-caelestia/#chapter-7-the-hibernation-swap-file), applied twice. There's no shared swap partition at all: zram handles ~all everyday memory pressure on either OS, and hibernation never has to share state across the boundary.
4. **UID/GID coordination for the shared partition.** If `youruser` is UID 1000 on Arch and UID 1001 on Ubuntu, every file on `/data` looks like it belongs to a different user depending on which OS you booted into. We pin UID 1000 on both sides during user creation.
...
The Arch side of this install begins exactly like the [basic install](../2026-05-01-first-arch-install/) and [Article 03](../2026-05-17-arch-luks-btrfs-caelestia/): verify the ISO, boot the live USB, get online, set the clock. Earlier drafts of this article just said *"do the first three steps like Article 03"* and moved on — but a dual-boot is exactly the kind of multi-hour job where you don't want to be flipping between three browser tabs to remember how `gpg --verify` goes. So this part is spelled out in full. **If you've done a from-scratch Arch install before, skim to [Chapter 1](#chapter-1-the-four-partition-layout).**
...
Mount the Arch root subvolumes with the right options — identical to [Article 03 Chapter 4](../2026-05-17-arch-luks-btrfs-caelestia/#chapter-4-mount-the-subvolumes-with-the-right-options):
...
Identical to [Article 03's Chapter 6](../2026-05-17-arch-luks-btrfs-caelestia/#chapter-6-locale-time-hostname-user) except for **one** dual-boot-specific change: we pin the user's UID and GID to `1000` explicitly. Ubuntu's installer assigns the first regular user UID 1000 by default, so if we also pin to 1000 here, files on `/data` will be owned by the same user-as-the-kernel-sees-it on both sides.
...
useradd -m -u 1000 -g 1000 -G wheel,video,audio,input youruser
passwd youruser
...
Replace `youruser` with whatever username you want; the only thing that matters is that you use the **same username and the same `-u 1000 -g 1000`** when you create the user during Ubuntu install later.

::: {.callout-important}
## The pinned UID is the load-bearing part of "shared data partition"

Linux filesystems store file ownership as integers, not names. If `youruser` is UID 1000 on Arch and `youruser` is UID 1001 on Ubuntu, the file ownership on `/data` literally cannot match in both directions — *somebody's* `ls -l /data` will show `1001` instead of `youruser`. Pin the UID at user-create time on both sides and the problem disappears before it starts.

If you forget to pin during Ubuntu install and the user ends up as UID 1001, the recovery is `usermod -u 1000 youruser && groupmod -g 1000 youruser && find / -uid 1001 -exec chown 1000 {} +` — workable but tedious. Pin in advance.
:::
...
Identical to [Article 03 Chapter 7](../2026-05-17-arch-luks-btrfs-caelestia/#chapter-7-the-hibernation-swap-file). zram (Chapter 10) handles everyday compressed swap; hibernation goes to a NoCOW swap file *inside* the Arch root, so the resume target is unambiguous and there's no collision with Ubuntu's own hibernation swap file.
...
Same as [Article 03 Chapter 8](../2026-05-17-arch-luks-btrfs-caelestia/#chapter-8-zram-for-everyday-swap):
...
The reasoning is identical to [Article 03 Chapter 9](../2026-05-17-arch-luks-btrfs-caelestia/#chapter-9-mkinitcpio--the-systemd-way) — only the Arch root container needs to be unlockable from the initramfs, and `sd-encrypt` + `rd.luks.name=…` is the path that does it.
...
# Should print: -rw-r--r-- 1 youruser youruser ... /data/probe-arch.txt
#                            ^^^^^^^^^^^^^ — user/group name, not raw UID
stat -c '%u %g' /data/probe-arch.txt
# Should print: 1000 1000
```

The point of the second `stat` line is to confirm the *integer* UID is 1000 — that's what Ubuntu's `youruser` will see when it mounts `/data` later. If the integer is anything other than 1000, your `useradd` in Chapter 6 didn't pin correctly; fix it now with `usermod -u 1000 youruser && chown -R 1000:1000 /home/youruser /data/*-by-youruser`.
...
6. **User account creation**: username `youruser` (match exactly what you used on Arch), tick **"Log in automatically"** off, password whatever. The installer does not expose the UID for the first user — it's hard-coded to 1000, which is exactly what we want.
7. Confirm, install.
...
Log in as `youruser`. Open a terminal:
...
ls -l /data                   # files written from Arch should appear, owned by youruser
```
...
ls -l /data                     # same files as Arch sees, owned by youruser
stat -c '%u %g %n' /data/*      # all UIDs/GIDs == 1000
```
...
Same four failure modes as Article 03's [emergency section](../2026-05-17-arch-luks-btrfs-caelestia/#emergency-timed-out-waiting-for-device):
...
```bash
sudo usermod -u 1000 youruser
sudo groupmod -g 1000 youruser
...
Reboot Ubuntu. Now `id youruser` prints `uid=1000(youruser) gid=1000(youruser)`.
