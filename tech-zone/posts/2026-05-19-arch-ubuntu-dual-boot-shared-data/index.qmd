...
1. **Who owns GRUB?** Both distros want to install their own bootloader to the same ESP. We pick one as the menu driver (Ubuntu) and let its `os-prober` discover Arch automatically.
...
| `p4`      | (remaining ÷ 2)    | Btrfs (inside LUKS2)        | LUKS2      | `/`        | **Ubuntu** root, passphrase-unlocked   |
...
You could do this differently: one LUKS2 container covering the entire post-ESP drive, then LVM inside it carving out logical volumes for Arch root, Ubuntu root, shared data, and swap. That gives you **one** passphrase prompt at boot.
...
We format two LUKS containers now (p2 = shared data, p3 = Arch root) and leave p4 untouched for Ubuntu. We LUKS-format p4 by hand during the manual Ubuntu install in Part 4: 24.04's installer can't encrypt a partition you hand it, so we drive `cryptsetup` ourselves there, giving Ubuntu's root its own container, keyslots, and `crypttab` UUID, fully separate from Arch's.
...
The mapper names matter, they're the contract between this step, the kernel cmdline, `/etc/crypttab`, and `/etc/fstab`. Pick a naming convention and stick with it. I'm using `crypt_arch` for the Arch root and `crypt_shared` for the data partition (Article 03 calls the Arch root `cryptroot`; here it becomes `crypt_arch` so the name itself says *which* OS's root it is, now that more than one is in play). Ubuntu's root is the third piece of the scheme: `crypt_ubuntu`. Because we install Ubuntu by hand later in this article (24.04's installer can't encrypt a partition you hand it), its mapper name is ours to choose too, so we `cryptsetup open` it as `crypt_ubuntu`, exactly parallel to Arch's `crypt_arch`. That's the name that goes into Ubuntu's own `/etc/crypttab` and `resume=` line.
...
Identical to [Article 03's Chapter 6](../2026-05-17-arch-luks-btrfs/#chapter-6-locale-time-hostname-user) except for **one** dual-boot-specific change: we pin the user's UID and GID to `1000` explicitly. We pin the Ubuntu user to 1000 the same way later (`useradd -u 1000 -g 1000`), so files on `/data` are owned by the same user-as-the-kernel-sees-it on both OSes. 1000 is the conventional first-user ID on both distros, but we set it by hand rather than assume it.
...
The `grub-install --bootloader-id=GRUB` writes a `GRUB` entry to the EFI NVRAM and copies the GRUB binaries to `/boot/EFI/GRUB/`. When you run Ubuntu's `grub-install` later (inside the manual install's chroot), it writes a separate `Ubuntu` NVRAM entry and copies its own binaries to `/boot/EFI/Ubuntu/`. The ESP holds **both** sets simultaneously, firmware decides which to chainload based on NVRAM boot-order.
...
Run that from the *Arch* side, before booting the Ubuntu live USB.
:::
...
::: {.callout-important}
## The `--modules=` list is the load-bearing part of this whole step

This is the single line that, if you get it wrong, lands you at a bare `grub>` prompt on the *next* boot with no menu and no obvious cause. Here's the mechanism: a plain `grub-install` writes an EFI core image (`/boot/efi/EFI/Ubuntu/grubx64.efi`) containing only the modules GRUB *thinks* it needs based on where its own `grub.cfg` lives. But our `grub.cfg` is on the **encrypted** `/boot`, which GRUB can't read until it has already loaded `cryptodisk` + `luks2` + the cipher modules (`gcry_rijndael`, `gcry_sha256`) + `pbkdf2` + the `btrfs` driver. That's a chicken-and-egg: the modules needed to *find* the config can't be loaded *from* the config. `--modules="…"` breaks the cycle by **baking those modules directly into the core image**, so they're present before GRUB looks for anything on disk.

Skip `--modules` (or let a paste split the line so it runs without the flag) and `grub-install` still prints `Installation finished. No error reported.` — it's the textbook false-success. The damage only shows at the next boot. If you land there, the [🚨 emergency section](#emergency-failure-modes) walks you back out from the `grub>` prompt and makes the fix permanent.
:::

::: {.callout-warning}
## Watch the paste: keep `grub-install` on one logical line

The `grub-install … --modules="…"` command is long enough that a terminal will visually wrap it, and if you paste into a console that submits on every newline, the `--modules="…"` half can run as a *separate* command — leaving the actual `grub-install` to run **without** the flag (the exact false-success above). The `\` line-continuation in the snippet keeps it one command; if you're hand-typing, type it as a single unbroken line, or set `M="part_gpt cryptodisk luks2 btrfs gcry_rijndael gcry_sha256 pbkdf2"` first and pass `--modules="$M"` so the long part is a short variable.
:::
...
cryptsetup luksAddKey --pbkdf pbkdf2 /dev/nvme0n1p4 /etc/cryptsetup-keys.d/root.key   # type the passphrase once
...
::: {.callout-warning}
## The `--pbkdf pbkdf2` on `luksAddKey` is not optional here

This is subtle and it *will* break your boot if you drop it. You formatted `p4` with `--pbkdf pbkdf2` back in step 1, so keyslot 0 (your passphrase) is GRUB-readable. But `luksAddKey` **ignores that** and derives each *new* keyslot with cryptsetup's own default, **Argon2id** — a memory-hard KDF GRUB 2.12 cannot process. The poison is worse than "GRUB can't use that slot": a single Argon2id keyslot anywhere on the device makes GRUB reject the **entire** container with `error: no cryptodisk module can handle this device`, so even your good PBKDF2 passphrase slot becomes unreachable and you're back at `grub>`. Forcing `--pbkdf pbkdf2` keeps keyslot 1 PBKDF2 too, so the whole header stays GRUB-friendly.

Verify *before you reboot* that no slot is Argon2id:

```bash
cryptsetup luksDump /dev/nvme0n1p4 | grep -E 'Keyslot|PBKDF:'
```

Every `PBKDF:` line must read `pbkdf2`. If you see `argon2id` on any keyslot, find its number in the dump and remove it — `cryptsetup luksKillSlot /dev/nvme0n1p4 <N>` — then re-add it with `--pbkdf pbkdf2`. Never kill the slot that holds your only passphrase.
:::
:::
...
Exit the chroot, then just reboot. Everything you needed is already written to disk; `reboot` flushes buffers and force-unmounts on its way down, which sidesteps a trap that bites here:
...
::: {.callout-note}
## Why we `reboot` instead of unmounting by hand

The tidy-looking `umount -R /mnt/run …; cryptsetup close crypt_ubuntu` sequence tends to fail with `target is busy`. The cause is the `mount --rbind /run /mnt/run` we did entering the chroot: `--rbind` is *recursive*, so it pulled `/run/user/0` and the live session's snap mount namespaces in under `/mnt/run`, and those keep the mount busy no matter how many times you retry `umount`. Chasing them down is pure ceremony, the config files are already on disk. `sync` flushes the write cache, `reboot` tears down every mount the hard way (including the busy ones) and closes the LUKS mappings as the kernel shuts down. Pull the Ubuntu USB as the laptop restarts.
:::
...
A from-scratch Ubuntu install has no hibernation swap file. We mirror Arch's Chapter 9 from inside the running Ubuntu system.
...
### 0. You boot Ubuntu and land at a bare `grub>` prompt (no menu)

This is the failure the `--modules=` and `--pbkdf pbkdf2` fixes in Part 4 exist to prevent, and the one most likely to bite if you installed before those fixes (or a paste split the `grub-install` line). The symptom is a minimal `grub>` shell instead of the boot menu: GRUB's EFI core loaded, but it can't read its own `grub.cfg` off the encrypted `/boot`. There are two root causes, and you can recover from the prompt and then make the fix permanent.

**First, get booted from the `grub>` prompt by unlocking the disk by hand.** Type these one at a time (GRUB has no copy-paste; if a line wraps, keep typing, don't press Enter mid-command):
...
- `cryptomount (hd0,gpt4)` unlocks the Ubuntu root LUKS partition (`p4` → `gpt4`); enter the Ubuntu passphrase.
- `ls` confirms the unlocked device appeared as `(crypto0)`. Use whatever name `ls` shows if it isn't `crypto0`.
- `configfile (crypto0)/@/boot/grub/grub.cfg` loads the real menu (`/@` is the Btrfs root subvolume). Pick **Ubuntu**, it boots.

If `cryptomount` itself errors with **`no cryptodisk module can handle this device`**, you have an **Argon2id keyslot** poisoning the header (the `--pbkdf pbkdf2` bug). You can't fix that from `grub>`, boot the **Ubuntu live USB** instead and inspect the slots:

```bash
sudo cryptsetup luksDump /dev/nvme0n1p4 | grep -E 'Keyslot|PBKDF:'
```

Any `PBKDF: argon2id` line is the culprit. Note its keyslot number, then remove **only that slot** (never the slot holding your only passphrase) and re-add it with `--pbkdf pbkdf2`:

```bash
sudo cryptsetup luksKillSlot /dev/nvme0n1p4 <ARGON2_SLOT_NUMBER>
# if it was the keyfile slot, re-add PBKDF2-style after booting (see Part 4's single-prompt callout)
```

**Then make the boot permanent from the now-running Ubuntu** (this is where `grub-install` detects the cryptodisk correctly, far better than it did in the chroot):
...
That rewrites the EFI core image *with* the crypto modules embedded. Reboot; you should get the menu (after GRUB's one passphrase prompt) with no `grub>` detour. If you'd rather not unlock-by-hand every boot until then, the auto-unlock instruction is what `grub-install` bakes in here, this single re-run is the fix.
...
If the Ubuntu user came out as UID 1001 (you skipped `-u 1000` on the `useradd`, or a UID 1000 already existed), fix it from Ubuntu:
