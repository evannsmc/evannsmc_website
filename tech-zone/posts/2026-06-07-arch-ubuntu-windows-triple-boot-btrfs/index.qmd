...
description: "The no-encryption sibling of the triple-boot finale: one NVMe drive booting Windows 11, Arch Linux, and Ubuntu 24 LTS, with both Linux roots on plain Btrfs (no LUKS) and one shared plain Btrfs /data partition the two Linux distros mount read-write. Windows goes on first so it can't stomp the bootloader; Windows↔Linux exchange uses a plain exFAT partition all three OSes read. Same Btrfs subvolumes, per-OS zram + NoCOW hibernation, one GRUB on the shared ESP with os-prober chainloading Windows. No passphrase prompts, no encrypted /boot, none of the grub> / Argon2 failure modes that come with GRUB unlocking LUKS. Secure Boot off, BitLocker off."
...
The plain-Btrfs twin of the [LUKS+Btrfs triple-boot (Article 10)](../2026-06-05-arch-ubuntu-windows-triple-boot/), built by removing every encryption layer from that design. It's a reasoned blueprint, not a command-by-command log I've executed end-to-end (yet). Dropping LUKS also drops the parts of the encrypted article that bit hardest in practice, the encrypted-`/boot` GRUB setup that can land you at a bare `grub>` prompt, the Argon2id-keyslot trap, the keyfile/`crypttab` dance, and the double passphrase prompt. The Windows-integration parts (install order, ESP sharing, the exFAT exchange, the RTC clock fix, Secure Boot / BitLocker) are unchanged, none of those depend on Linux encryption.
...
The same triple-boot as [Article 10](../2026-06-05-arch-ubuntu-windows-triple-boot/), minus encryption:

- **Windows 11**, installed **first**, on its own NTFS partition, so it can't clobber a bootloader that isn't there yet.
- **Arch Linux** and **Ubuntu 24 LTS**, each on its **own plain Btrfs root** (no LUKS) with the usual subvolumes (`@`, `@home`, `@snapshots`, `@var_log`, `@swap`), exactly the [plain-Btrfs dual-boot](../2026-06-07-arch-ubuntu-dual-boot-btrfs/), unchanged.
...
Seven partitions on one drive, the same shape as the encrypted triple-boot minus the LUKS column:
...
If you want to SSH in from another machine for comfort, it's the same `iwctl` → `systemctl enable --now sshd` → `passwd` dance as the [dual-boot Chapter 0](../2026-06-07-arch-ubuntu-dual-boot-btrfs/#chapter-0-verify-the-iso-boot-the-live-usb-get-online).
...
Usually a firmware still in CSM/legacy mode, or a stray boot flag. Re-check that firmware is **UEFI-only** (Part 2). If the installer insists on its own partitioning, let it install to `p3` and create whatever MSR/recovery partitions it wants *inside the space we gave it*, but never let it touch `p1` (ESP) or the Linux partitions.
:::
...
Boot the Arch live USB again. From here, the Arch install is **identical to the [plain-Btrfs dual-boot](../2026-06-07-arch-ubuntu-dual-boot-btrfs/)**, same Btrfs subvolumes, same shared `/data`, same per-OS hibernation, with one difference: the partitions already exist and Windows is present, so we **don't** repartition. Every command is inline; for the deep *why*, the dual-boot article's matching chapter is linked.

## Chapter 5: Format the two Arch-side Btrfs filesystems

`p5` is the shared data partition; `p6` is the Arch root. Leave `p7` for Ubuntu. No `cryptsetup`, we `mkfs.btrfs` straight onto the partitions:
...
[Subvolume rationale →](../2026-06-07-arch-ubuntu-dual-boot-btrfs/#chapter-3-create-the-btrfs-subvolumes-on-the-arch-root)
...
`@swap` mounts **without** compression. We mount `/data` now so `genfstab` captures it, but **don't** add the exFAT exchange to `genfstab`'s view, we'll hand-write its `fstab` line in Chapter 9 (exFAT needs ownership options Btrfs doesn't). [Mount-option rationale →](../2026-06-07-arch-ubuntu-dual-boot-btrfs/#chapter-4-mount-everything-for-pacstrap)

## Chapter 8: Pacstrap, fstab, chroot

Same package set as the dual-boot (note: **no `cryptsetup`**), plus **`exfatprogs`** for the exchange partition:
...
Use `amd-ucode` on AMD hardware. The two additions vs. a single-boot install are **`os-prober`** (so GRUB discovers Windows *and* Ubuntu) and **`exfatprogs`** (so Arch can mount the exchange). [fstab sanity check →](../2026-06-07-arch-ubuntu-dual-boot-btrfs/#chapter-5-pacstrap-fstab-chroot)

## Chapter 9: Configure the system inside the chroot

This folds together the dual-boot's Chapters 6–10, locale/user (pinned UID), the `/data` hardening, hibernation, zram, the exchange-partition `fstab` line, and `mkinitcpio`. Each block is the same as the dual-boot; run them in order.
...
The **pinned UID/GID 1000** is what makes file ownership on the shared `/data` match between Arch and Ubuntu. Use the **same username and UID** when you create the Ubuntu user later. [Why the pinned UID is load-bearing →](../2026-06-07-arch-ubuntu-dual-boot-btrfs/#chapter-6-locale-time-hostname-user-with-a-pinned-uid)

### Harden the shared `/data` and hand it to your user

No keyfile, no `crypttab`, the shared partition is plain Btrfs `genfstab` already recorded. Just soft-fail the mount and fix ownership:
...
Ownership is stored in the filesystem, so Ubuntu (also UID 1000) will see these files as its own user automatically, no second `chown` from Ubuntu. [Full `/data` reasoning →](../2026-06-07-arch-ubuntu-dual-boot-btrfs/#chapter-7-harden-the-data-fstab-line-and-hand-it-to-your-user)
...
Don't `swapon` from inside the chroot (it blocks clean unmounting). zram (priority 100) carries everyday swap; the swap file (priority -2) is for hibernation. [Two-tier swap rationale →](../2026-06-07-arch-ubuntu-dual-boot-btrfs/#chapter-9-zram-for-everyday-compressed-swap)

### mkinitcpio

Edit `/etc/mkinitcpio.conf`, note the `HOOKS` line has **no `sd-encrypt`** (there's no LUKS root to unlock):
...
No `/etc/crypttab.initramfs` to write, the root mounts directly from `p6`. [mkinitcpio reasoning →](../2026-06-07-arch-ubuntu-dual-boot-btrfs/#chapter-10-mkinitcpio-and-the-grub-cmdline)

## Chapter 10: GRUB, cmdline, os-prober, install

Get the **Arch root Btrfs filesystem's UUID** (`p6`):

```bash
blkid -s UUID -o value /dev/nvme0n1p6
```

Edit `/etc/default/grub`. Without LUKS there's no `rd.luks.name=` and no `/dev/mapper/…` root, `grub-mkconfig` auto-detects the root and `subvol=@`, so we only add the hibernation resume target (substitute real values, **no angle brackets in the file**):
...
::: {.callout-note}
## No `GRUB_ENABLE_CRYPTODISK`, no `--modules`, no `grub>` trap

This `grub-install` is a plain one, no crypto modules to embed, no `GRUB_ENABLE_CRYPTODISK=y`. `/boot` is the unencrypted FAT32 ESP, so GRUB reads kernels and `grub.cfg` directly. The entire "GRUB drops to `grub>` because it can't unlock encrypted `/boot`" failure class from the encrypted article cannot occur here. [Why no cryptodisk →](../2026-06-07-arch-ubuntu-dual-boot-btrfs/#chapter-10-mkinitcpio-and-the-grub-cmdline)
:::
...
There's no `/etc/crypttab` to check and no keyfile to list. Leave the chroot and reboot:
...
No `cryptsetup close`, nothing's open. Pull the USB. Expected first boot: firmware → **GRUB menu** (listing *Arch Linux* and *Windows Boot Manager*) → pick Arch → kernel/initramfs → **no passphrase prompt** → `@` mounts as `/` → `/data` and `/exchange` mount, zram comes up → login. If you drop into emergency mode, jump to the [🚨 emergency section](#emergency-failure-modes).
...
Identical to the [plain-Btrfs dual-boot Part 4](../2026-06-07-arch-ubuntu-dual-boot-btrfs/#part-4-install-ubuntu-24-lts-alongside), with Windows already present (which changes nothing about the Ubuntu steps, Ubuntu's `os-prober` will simply find both Arch *and* Windows).
...
Boot a verified Ubuntu 24.04 LTS Desktop USB and pick **"Try Ubuntu"**. We build Ubuntu's root by hand from the live session with `debootstrap`, exactly as in the dual-boot, with **p7** as Ubuntu's root.

::: {.callout-tip}
## Driving it remotely over SSH?

The Ubuntu Desktop live image has no SSH server and logs you in as a passwordless `ubuntu` user, so it needs a few setup commands first, do it exactly as in the [dual-boot SSH-in callout](../2026-06-07-arch-ubuntu-dual-boot-btrfs/#install-ubuntu-by-hand-from-the-live-session) (`apt install openssh-server`, `passwd ubuntu`, `systemctl enable --now ssh`, then `ssh ubuntu@<ip>` + `sudo -i`).
:::

Open a terminal, `sudo -i`, `apt update && apt install -y debootstrap` (no `cryptsetup-bin` needed), then:

```bash
# 1 — format p7 as Btrfs + subvolumes (no LUKS)
...
# 3 — bootstrap + fstab (root, home, ESP, shared /data — all plain, no crypttab)
...
Inside the chroot, widen the sources list, install the system (note: **no `cryptsetup`/`cryptsetup-initramfs`**, and a plain `grub-install`), and create the UID-1000 user, **identical to [dual-boot Part 4 step 5](../2026-06-07-arch-ubuntu-dual-boot-btrfs/#install-ubuntu-by-hand-from-the-live-session)** (root device here is `/dev/nvme0n1p7`):
...
::: {.callout-note}
## Notice the plain `grub-install`

No `GRUB_ENABLE_CRYPTODISK=y`, no `--modules="part_gpt cryptodisk luks2 …"`, no root `crypttab` entry. Ubuntu's `/boot/efi` is the shared unencrypted ESP and its kernel lives on a plain Btrfs root GRUB reaches normally. The `grub>` prompt the encrypted article's `--modules` step exists to prevent simply can't happen here.
:::
...
Point Ubuntu's cmdline and initramfs at its *own* Btrfs filesystem (never Arch's):

```bash
sudo blkid -s UUID -o value /dev/nvme0n1p7    # <UBUNTU_BTRFS_UUID>
...
If Part 8 passed, skip this. The plain-Btrfs dual-boot's [emergency section](../2026-06-07-arch-ubuntu-dual-boot-btrfs/#emergency-failure-modes) covers the Btrfs/`/data`/hibernation failures; the ones below are the Windows-specific additions.
:::

::: {.callout-tip}
## No `grub>` prompt to recover from

Because both Linux distros keep `/boot` on the plain FAT32 ESP, the bare-`grub>` failure that the *encrypted* triple-boot could hit (GRUB unable to unlock the encrypted `/boot`) **cannot happen here.** GRUB always reads its config and kernels normally.
:::
...
The tradeoff you made is the one Part 1 spelled out: **nothing on this disk is encrypted at rest.** That's the right call for a stationary, physically-secured machine and the wrong one for a laptop. If the machine's role changes, the [encrypted triple-boot (Article 10)](../2026-06-05-arch-ubuntu-windows-triple-boot/) is the same layout with LUKS2 around each Linux root and the shared `/data`, and the hardening this no-encryption build let you skip (the `--modules` EFI-core surgery, the `--pbkdf pbkdf2` keyslot discipline, the `grub>` recovery) is exactly what that article now documents in full.
