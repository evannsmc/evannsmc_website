...
description: "One NVMe drive booting Windows 11, Arch Linux, and Ubuntu 24 LTS, with both Linux roots on plain Btrfs (no LUKS) and one shared plain Btrfs /data partition the two Linux distros mount read-write. Windows goes on first so it can't stomp the bootloader; Windows↔Linux exchange uses a plain exFAT partition all three OSes read. Btrfs subvolumes, per-OS zram + NoCOW hibernation, one GRUB on the shared ESP with os-prober chainloading Windows. No passphrase prompts, no encrypted /boot, none of the grub> / Argon2 failure modes that come with GRUB unlocking LUKS. Secure Boot off, BitLocker off."
...
A no-encryption triple-boot blueprint: Windows 11, Arch, and Ubuntu on one drive, with both Linux roots on plain Btrfs and a shared Btrfs `/data`, no LUKS anywhere. It's a carefully-reasoned procedure, not a command-by-command log I've executed end-to-end (yet). Because the disk is unencrypted, `/boot` sits on a plain ESP that GRUB reads directly — none of the `grub>`-prompt, Argon2id-keyslot, or keyfile/`crypttab` failure modes that come with an encrypted `/boot` can happen here. The Windows-integration parts (install order, ESP sharing, the exFAT exchange, the RTC clock fix, Secure Boot / BitLocker) are pure Windows concerns — encryption never touches them.
...
- **Arch Linux** and **Ubuntu 24 LTS**, each on its **own plain Btrfs root** (no LUKS) with the usual subvolumes (`@`, `@home`, `@snapshots`, `@var_log`, `@swap`) — the same root layout as the [plain-Btrfs dual-boot](../2026-06-07-arch-ubuntu-dual-boot-btrfs/).
...
Adding Windows introduces five concerns a Linux-only dual-boot doesn't have — all about Windows, not about encryption:
...
The package set — note there's **no `cryptsetup`** (nothing here manages a LUKS container), plus **`exfatprogs`** for the exchange partition:
...
`MODULES=(btrfs)` lets the initramfs read a Btrfs root. The `HOOKS` line is the standard `systemd` set with **no `sd-encrypt`** — that hook exists only to unlock a LUKS root from the initramfs, and there's no encrypted root here. The root mounts directly from `p6`, so there's also no `/etc/crypttab.initramfs` to write. Save, then rebuild:
...
The resume target is the only thing you add by hand. `grub-mkconfig` auto-detects the root partition and `subvol=@`, and with no LUKS there's no `rd.luks.name=` or `/dev/mapper/…` root to specify.
...
This is a plain `grub-install`: no crypto modules to embed, no `GRUB_ENABLE_CRYPTODISK=y`. Because `/boot` is the unencrypted FAT32 ESP, GRUB reads its kernels and `grub.cfg` directly — the "GRUB drops to `grub>` because it can't unlock an encrypted `/boot`" failure class needs an encrypted `/boot` to happen, and there isn't one here.
:::
...
Inside the chroot, widen the sources list, install the system (note: **no `cryptsetup`/`cryptsetup-initramfs`**, and a plain `grub-install` — there's no encrypted `/boot` to teach GRUB to unlock), and create the UID-1000 user (root device here is `/dev/nvme0n1p7`):
...
No `GRUB_ENABLE_CRYPTODISK=y`, no `--modules="part_gpt cryptodisk luks2 …"`, no root `crypttab` entry. Ubuntu's `/boot/efi` is the shared unencrypted ESP, and its kernel lives on a plain Btrfs root GRUB reaches normally. A bare `grub-install` is correct and complete — the crypto-module embedding exists only to let GRUB read an encrypted `/boot`, which this layout doesn't have.
:::
...
Because both Linux distros keep `/boot` on the plain FAT32 ESP, the bare-`grub>` failure that comes with an encrypted `/boot` (GRUB unable to unlock it before it can read the kernel) **cannot happen here.** GRUB always reads its config and kernels normally.
:::
...
The tradeoff you made is the one Part 1 spelled out: **nothing on this disk is encrypted at rest.** That's the right call for a stationary, physically-secured machine and the wrong one for a laptop. If the machine's role changes, adding encryption means rebuilding on LUKS2 — the same layout, but with a LUKS2 container around each Linux root and the shared `/data`. That brings back the hardening this build let you skip: the `--modules` EFI-core surgery so GRUB can read an encrypted `/boot`, the `--pbkdf pbkdf2` keyslot discipline, and a `grub>` recovery path. There's no in-place way to add it; decide up front.
