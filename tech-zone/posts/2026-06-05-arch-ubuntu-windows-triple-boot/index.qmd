...
cryptsetup open /dev/nvme0n1p5 cryptshared
...
echo "cryptshared UUID=$(blkid -s UUID -o value /dev/nvme0n1p5) /etc/cryptsetup-keys.d/shared.key luks,nofail" \
    >> /etc/crypttab
...
```bash
echo "cryptroot UUID=$(blkid -s UUID -o value /dev/nvme0n1p6) none luks" > /etc/crypttab.initramfs
mkinitcpio -P
```
...
cat /etc/fstab    | grep -E 'subvol=|swap|cryptshared|exchange'
...
cryptsetup status cryptroot      # active LUKS2 (Arch root)
cryptsetup status cryptshared    # active LUKS2 (shared data)
```
...
ls -l /exchange/from-arch.txt                          # owned by youruser (uid=1000 from the mount options)
...
4. **User account**: username `youruser` (match Arch exactly). The installer hard-codes the first user to UID 1000 — exactly what we want for `/data` ownership parity.
...
ls -l /data /exchange            # files written from Arch appear, owned by youruser
...
cryptsetup status cryptshared       # active LUKS2 (same shared UUID from both)
