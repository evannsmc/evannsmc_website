...
echo "cryptshared UUID=$(blkid -s UUID -o value /dev/nvme0n1p5) /etc/cryptsetup-keys.d/shared.key luks" \
    >> /etc/crypttab
```

The keyfile lives inside the encrypted Arch root, so it's only readable once that root is unlocked — the shared partition unlocks silently *after* the root mounts, no second passphrase prompt. We copy the same keyfile onto the Ubuntu root later. [Keyfile boot-sequence walkthrough →](../2026-05-19-arch-ubuntu-dual-boot-shared-data/#chapter-7-generate-the-shared-keyfile-and-add-it-to-the-shared-luks-container)
