...
description: "The no-encryption sibling of the LUKS+Btrfs dual-boot: one NVMe drive booting Arch Linux and Ubuntu 24 LTS, each on its own plain Btrfs root (no LUKS), plus one shared 150 GiB plain Btrfs data partition both distros mount read-write. Same Btrfs subvolume layout (`@`, `@home`, `@snapshots`, `@var_log`, `@swap`), same per-OS zram + NoCOW Btrfs hibernation, same pinned-UID shared `/data`, but with every cryptsetup, keyfile, crypttab, and GRUB-cryptodisk step removed. No passphrase prompts, no encrypted `/boot`, and none of the `grub>` failure modes that come with GRUB unlocking LUKS. GRUB on the single shared ESP, with os-prober enabled."
...
::: {.callout-warning}
## Disclaimer: this is the reasoned no-encryption variant, not yet a live-tested run

This is the plain-Btrfs twin of the [LUKS+Btrfs dual-boot (Article 07)](../2026-05-19-arch-ubuntu-dual-boot-shared-data/), built by taking that working design and **removing every encryption layer**. It's a reasoned blueprint, not a command-by-command log I've executed end-to-end (yet). The upside of dropping LUKS is that it also drops the parts of Article 07 that bit hardest in practice: the encrypted-`/boot` GRUB setup that can land you at a bare `grub>` prompt, the Argon2id-keyslot trap, the keyfile/`crypttab` dance, and the double passphrase prompt. What's left is the boring, dependable shape, and "boring and dependable" is exactly what you want from a bootloader.
...
The same dual-boot layout as [Article 07](../2026-05-19-arch-ubuntu-dual-boot-shared-data/), minus encryption:

- **Two plain Btrfs root partitions** — Arch in its own Btrfs partition, Ubuntu in its own, each carrying the usual subvolumes (`@`, `@home`, …). No LUKS container around either.
- **One shared 150 GiB plain Btrfs data partition**, mounted at `/data` on both distros, owned by your user on both sides. Just an `fstab` UUID line on each OS, no keyfile, no `crypttab`, no auto-unlock machinery.
...
It stops on the console for the Arch side; the desktop layer is your choice, exactly like the encrypted articles. Ubuntu's GNOME comes up out of the box.
:::
...
If you want encryption, use [Article 07](../2026-05-19-arch-ubuntu-dual-boot-shared-data/) instead, it's the same design with LUKS2 around each root and the shared partition. This article is for when you've decided you don't.
...
If any of that is unacceptable for what lives on this machine, stop and use the encrypted article. Everything below assumes you've consciously chosen an unencrypted disk.
:::

What adding Ubuntu still introduces (encryption or not):
...
This is the central design decision. Four partitions on one drive, the same shape as the encrypted article minus the LUKS column:
...
No `cryptsetup` step here, that's the whole point. We format the Btrfs filesystems directly on the partitions. Format the ESP, the shared data partition, and the Arch root now; leave `p4` for the Ubuntu install in Part 4.
...
In the encrypted article this step ran `mkfs.btrfs` against a `/dev/mapper/...` device (the unlocked LUKS container); here it runs against the **raw partition**, because there's no LUKS layer in between. Simpler, and one fewer place to get the device name wrong.

---

## Chapter 3: Create the Btrfs subvolumes on the Arch root

Same five-subvolume layout as the encrypted article, the kernel doesn't care about the names, but snapper does:
...
The `@swap` subvolume mounts **without** compression, a swap file must not be compressed. We mount `/mnt/data` *now* so `genfstab -U /mnt` in the next chapter captures the shared partition automatically, and since there's no `crypttab` to write, that's the *only* thing we have to do for `/data` on the Arch side (plus a one-line `fstab` hardening and a `chown`, Chapter 7).

---

## Chapter 5: Pacstrap, fstab, chroot

Note what's **not** in this package list versus the encrypted article: **no `cryptsetup`**. Nothing here manages a LUKS container, so the tool isn't needed.
...
The two distinct Btrfs UUIDs (Arch root vs. shared data) confirm `genfstab` saw them as separate filesystems. If you see only one Btrfs UUID, you skipped a mount somewhere; re-run Chapter 4. (In this unencrypted layout, these UUIDs are the *partitions'* own Btrfs UUIDs, there's no LUKS UUID to keep separate, one fewer identifier to track than in Article 07.)
:::
...
This is where the encrypted article had three chapters (keyfile, `crypttab`, `crypttab.initramfs`). With no encryption, **all of that collapses into two small edits to a partition that's already in `fstab`.** There's no container to unlock, so there's nothing to unlock it *with*, the shared partition is just a Btrfs filesystem `genfstab` already recorded.
...
::: {.callout-tip}
## `nofail` is cheap insurance even without encryption

In Article 07, `nofail` was the fix for "my keyfile-unlocked partition dropped me to emergency mode." There's no keyfile here, but the principle still applies to *any* secondary mount: if the shared partition is ever absent (a future repartition, a failing drive, a typo'd UUID), you want a normal login you can debug from, not an emergency shell. `nofail` buys exactly that for one word.
:::
...
In the encrypted article, `resume=` pointed at `/dev/mapper/crypt_arch` (the unlocked container). Here the swap file lives directly on the Arch Btrfs filesystem, so the resume device is just that filesystem, addressed by its UUID. We'll write `resume=UUID=<arch-btrfs-uuid>` in the cmdline, no mapper indirection.
:::
...
Here's a concrete simplification from dropping encryption. Open `/etc/mkinitcpio.conf` and set:
...
The encrypted article's `HOOKS` line included **`sd-encrypt`**, the hook that prompts for the LUKS passphrase and unlocks the root from the initramfs. **We remove it** — there's no container to unlock, so the root is mounted directly from the partition. That also means no `/etc/crypttab.initramfs` to write. Regenerate the initramfs:
...
Open `/etc/default/grub` and set the `GRUB_CMDLINE_LINUX_DEFAULT` line. Without LUKS there's no `rd.luks.name=…` and no `/dev/mapper/…` root, `grub-mkconfig` auto-detects the root partition and `subvol=@` for you, so the only thing we *must* add by hand is the hibernation resume target:

```bash
GRUB_CMDLINE_LINUX_DEFAULT="loglevel=3 quiet zswap.enabled=0 resume=UUID=<ARCH_BTRFS_UUID> resume_offset=<RESUME_OFFSET>"
```

Get `<ARCH_BTRFS_UUID>` (the **Btrfs filesystem** UUID of the Arch root, the same one `genfstab` used, *not* a partition or LUKS UUID):

```bash
blkid -s UUID -o value /dev/nvme0n1p3
```

Replace `<ARCH_BTRFS_UUID>` and `<RESUME_OFFSET>` with the real values. **Do not include the angle brackets in the file.**
...
Note what's *gone* compared to Article 07: no `rd.luks.name=`, no `root=/dev/mapper/…`. GRUB's generated `linux` line carries `root=UUID=…` and `rootflags=subvol=@` automatically because it reads them off the mounted root at `grub-mkconfig` time.
...
::: {.callout-note}
## No `--modules`, no `GRUB_ENABLE_CRYPTODISK`, no `grub>` trap

This is the single biggest reliability win of the unencrypted layout. In the encrypted article, Ubuntu's `/boot` lived *inside* a LUKS container, so GRUB had to unlock it to read the kernel, which meant `GRUB_ENABLE_CRYPTODISK=y` and embedding `cryptodisk`/`luks2`/cipher modules into the EFI core image with `grub-install --modules="…"`. Get that wrong (or let a paste split the line) and you land at a bare `grub>` prompt with no menu.

Here, **both** distros keep `/boot` on the unencrypted FAT32 ESP, exactly the standard arrangement GRUB is designed for. GRUB reads the kernel and `grub.cfg` directly, no crypto modules, no core-image surgery, no Argon2id keyslot to poison the header. The entire `grub>` failure class from Article 07 simply cannot occur.
:::
...
Note there's **no `HOOKS=… sd-encrypt …`**, no `/etc/crypttab`, and no keyfile to list, those are the lines the encrypted article had and this one doesn't.
...
There's no `cryptsetup close` step, there are no open containers. Pull the USB. Expected first-boot sequence:

1. Firmware → GRUB menu (just `Arch Linux` for now).
2. Kernel + initramfs.
3. **No passphrase prompt**, the root mounts directly. (This is the visible payoff of dropping encryption: boot goes straight from GRUB to login.)
...
The reason Article 07 builds Ubuntu by hand is that 24.04's rewritten installer **dropped on-partition encryption**. Without encryption, that objection goes away, and the GUI installer's manual ("Something else") mode *can* format `p4` as Btrfs and mount it at `/`. But it won't reliably create the `@`/`@home` subvolume layout we want, and it likes to reorder things on the ESP. So we keep the **same by-hand `debootstrap` install** as the encrypted article (minus the crypto): it gives an exact-parity subvolume layout, uses the same tooling you already know, and never surprises you. If you genuinely don't care about subvolume parity, the GUI manual mode is a legitimate shortcut here, just point it at `p4`, choose Btrfs and mount point `/`, and reuse (don't reformat) the existing ESP at `/boot/efi`.
:::
...
Note we **don't** install `cryptsetup-bin`, there's nothing to encrypt.

**1 — Format p4 as Btrfs and create the subvolumes.** No `cryptsetup luksFormat`, no `cryptsetup open`, we `mkfs.btrfs` straight onto the partition:
...
**3 — Bootstrap a minimal Ubuntu 24.04 (`noble`) and write its `fstab`** (including the shared `/data` line, which on the Ubuntu side is *just* an `fstab` entry, no `crypttab`):
...
**5 — Inside the chroot: sources, kernel, GRUB, user, desktop.** Widen the `debootstrap` `main`-only sources list so `ubuntu-desktop` resolves, then install. Note the package list: **no `cryptsetup`, no `cryptsetup-initramfs`** — neither is needed:
...
::: {.callout-note}
## Notice what this `grub-install` *doesn't* need

Compare this line to the encrypted article's: there's **no `GRUB_ENABLE_CRYPTODISK=y`**, **no `--modules="part_gpt cryptodisk luks2 …"`**, and **no `crypttab` entry for the root**. Ubuntu's `/boot/efi` is the shared unencrypted ESP and its kernel/initramfs live on a plain Btrfs root GRUB reaches normally. A bare `grub-install` is correct and complete, the chicken-and-egg "GRUB can't read its own config off the encrypted `/boot`" problem doesn't exist here, so neither does the `grub>` prompt it caused.
:::
...
Edit `/etc/default/grub`. Ubuntu's `GRUB_CMDLINE_LINUX_DEFAULT` typically reads `quiet splash`. Add `resume=` and `resume_offset=`, using the **Ubuntu Btrfs filesystem's** UUID (`p4`) and the offset from the previous step:

```bash
sudo blkid -s UUID -o value /dev/nvme0n1p4     # save this as <UBUNTU_BTRFS_UUID>
...
Confirm Ubuntu sees the shared partition and its own swap:

```bash
findmnt /data                 # /dev/nvme0n1p2, subvol=@data
swapon --show                 # zram (Ubuntu sets it up automatically) + /swap/swapfile
...
::: {.callout-tip}
## The good news: no `grub>` prompt to recover from

The most painful failure of the *encrypted* dual-boot, GRUB dropping to a bare `grub>` prompt because it couldn't unlock the encrypted `/boot`, **cannot happen here.** Both distros' `/boot` is the plain FAT32 ESP, so GRUB always reads its config and kernels normally. If you came here from Article 07's troubleshooting, that entire section doesn't apply to this layout.
:::
...
With no LUKS, the usual causes narrow to:
...
If this machine's role changes (it becomes a laptop, it starts holding sensitive data), the upgrade path is the [encrypted dual-boot (Article 07)](../2026-05-19-arch-ubuntu-dual-boot-shared-data/), same subvolume layout, same shared-`/data` idea, with LUKS2 around each root and the data partition. There's no in-place "add encryption" shortcut on Linux; it means backing up, re-formatting the roots inside LUKS containers, and restoring. Better to decide up front, which is exactly why this article spells out the tradeoff in Part 1.
:::
