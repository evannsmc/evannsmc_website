...
Selecting Ubuntu briefly shows Ubuntu's *own* GRUB before it boots — the chainload working as designed. To boot straight through, set in Ubuntu's `/etc/default/grub`: `GRUB_TIMEOUT=0` and `GRUB_TIMEOUT_STYLE=hidden`, then `sudo update-grub`. Hold `Shift`/`Esc` during the hop if you ever need Ubuntu's recovery entries.
:::
