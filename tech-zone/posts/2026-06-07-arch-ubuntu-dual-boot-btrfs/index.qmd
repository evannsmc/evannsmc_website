...
description: "One NVMe drive booting Arch Linux and Ubuntu 24 LTS, each on its own plain Btrfs root (no LUKS), plus one shared 150 GiB plain Btrfs data partition both distros mount read-write. A Btrfs subvolume layout (`@`, `@home`, `@snapshots`, `@var_log`, `@swap`), per-OS zram + NoCOW Btrfs hibernation, and a pinned-UID shared `/data` — with no cryptsetup, keyfile, crypttab, or GRUB-cryptodisk anywhere. No passphrase prompts, no encrypted `/boot`, and none of the `grub>` failure modes that come with GRUB unlocking LUKS. GRUB on the single shared ESP, with os-prober enabled."
...
This is a no-encryption dual-boot blueprint: two plain Btrfs roots and a shared Btrfs `/data`, with no LUKS anywhere on the disk. It's a carefully-reasoned procedure, not a command-by-command log I've executed end-to-end (yet). Because the disk is unencrypted, `/boot` sits on a plain ESP and GRUB reads its config and kernels directly — none of the `grub>`-prompt, Argon2id-keyslot, or keyfile/`crypttab` failure modes that come with an encrypted `/boot` can happen here. What's left is the boring, dependable shape, and "boring and dependable" is exactly what you want from a bootloader.
...
Each `mkfs.btrfs` runs directly on the **raw partition** — with no LUKS layer in between, the device name is simply the partition itself. The `-L` labels (`shared`, `arch`) are cosmetic, but they make later `lsblk -f` and `blkid` output easy to read at a glance.
...
There's deliberately **no `cryptsetup`** in this package list — nothing here manages a LUKS container, so the tool isn't needed.
...
Sharing `/data` between the two distros is **two small edits to a line that's already in `fstab`.** There's no container to unlock and nothing to unlock it *with* — the shared partition is a plain Btrfs filesystem that `genfstab` already recorded when you generated the file back in Chapter 5.
...
The swap file lives directly on the Arch Btrfs filesystem, so the resume device *is* that filesystem, addressed by its UUID. We'll write `resume=UUID=<arch-btrfs-uuid>` in the GRUB cmdline (Chapter 10) — no `/dev/mapper` indirection, because there's no container in the path. The kernel finds the hibernation image from two values: the filesystem UUID (`resume=`) and the byte offset of the swap file's first extent (`resume_offset=`, the integer you just saved).
:::
...
`MODULES=(btrfs)` makes sure the initramfs can read a Btrfs root. The `HOOKS` line is the standard `systemd`-based set with **no `sd-encrypt`** in it — that hook exists only to prompt for a LUKS passphrase and unlock an encrypted root from the initramfs, and there's no encrypted root here. The root mounts directly from the partition, which also means there's no `/etc/crypttab.initramfs` to write. Save, then regenerate the initramfs:
...
The resume target is the only thing you must add by hand. `grub-mkconfig` auto-detects the root partition and `subvol=@` for you, and with no LUKS there's no `rd.luks.name=…` and no `/dev/mapper/…` root to specify.
...
GRUB's generated `linux` line carries `root=UUID=…` and `rootflags=subvol=@` automatically, because `grub-mkconfig` reads them off the mounted root when it generates the config. There's no `rd.luks.name=` and no `root=/dev/mapper/…` to add — those exist only when the root sits inside a LUKS container.
...
This is the single biggest reliability win of the unencrypted layout. When `/boot` lives inside a LUKS container, GRUB has to unlock it before it can even read the kernel — which forces `GRUB_ENABLE_CRYPTODISK=y` and embedding `cryptodisk`/`luks2`/cipher modules into the EFI core image with `grub-install --modules="…"`. Get that line wrong (or let a paste split it) and you land at a bare `grub>` prompt with no menu.

Here, **both** distros keep `/boot` on the unencrypted FAT32 ESP — exactly the standard arrangement GRUB is designed for. A plain `grub-install` is all you need: GRUB reads the kernel and `grub.cfg` directly, with no crypto modules, no core-image surgery, and no Argon2id keyslot to poison the header. That entire `grub>` failure class simply cannot occur on this layout.
:::
...
Note there's **no `sd-encrypt` in `HOOKS`**, no `/etc/crypttab`, and no keyfile anywhere — an unencrypted system simply has no such lines to carry.
...
There's **no `GRUB_ENABLE_CRYPTODISK=y`**, **no `--modules="part_gpt cryptodisk luks2 …"`**, and **no `crypttab` entry for the root**. Ubuntu's `/boot/efi` is the shared unencrypted ESP, and its kernel and initramfs live on a plain Btrfs root that GRUB reaches normally. A bare `grub-install` is correct and complete here — the chicken-and-egg "GRUB can't read its own config off the encrypted `/boot`" problem only exists with an encrypted `/boot`, so neither it nor the `grub>` prompt it causes can happen.
:::
...
The classic painful multi-boot failure — GRUB dropping to a bare `grub>` prompt because it couldn't unlock an encrypted `/boot` — **cannot happen here.** Both distros' `/boot` is the plain FAT32 ESP, so GRUB always reads its config and kernels normally. That whole failure class is simply off the table on an unencrypted layout.
:::
