...
- For this variant I'm assuming you want an unencrypted regular partition swap-only (no hibernation). If you want hibernation-safe encrypted swap, see [Article 01's plan](../2026-05-05-arch-luks-btrfs-lessons/), but be aware Article 01 didn't boot, so use the [Article 03 swap-file-in-LUKS pattern](../2026-05-17-arch-luks-btrfs/) instead (which lives in the `LUKS + Btrfs` tab on this page).
